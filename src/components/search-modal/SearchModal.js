  ...
  if (searchShow) {
   return (
    <div>
      ...
      const {_id, title} = oppo;
       return (
        <div className='d-flex align-items-center suggestions' style={{overflowY: 'auto'}}>
              <Link to='/searchitems'>
                <FaSistrix className='m-2'/>
                <span key={_id} onClick={()=> edit(_id)}>{title}</span>
              </Link>
              </div>
       )
     })}
  ...
  <div className={searchModal ? " search-modal show-modal d-md-none" : "search-modal"}>
    <div className="row modal-search-bar">
      <div className="col-12 d-flex align-items-center">
        ...
        <input type="search" value={name} onChange={handleChange} placeholder='Search products...' className="modal-input w-100"/>
        <Link to='/searchitems'>
